...
async function loadPlaywright() {
  ...
  throw new Error(
    'Playwright is not available. Install dependencies with npm install, or run an existing capture tool once to populate the shared cache.',
  );
}
...
Usage:
  npm exec -- node scripts/capture-theme-source.mjs https://example.com --theme example
  npm exec -- node scripts/capture-theme-source.mjs --url https://example.com --theme example -o .local/theme-capture-example
